#include "actors/actor_Clip_source.h"
...
bool read_signed_guard (actor_Clip_source *self) { 
      int s = ReadToken(self->SIGNED, 1);

  return (count < 0);  }
bool limit_max_guard (actor_Clip_source *self) { 
      int i = ReadToken(self->I, 1);

  return (i > 255);  }
bool limit_zero_guard (actor_Clip_source *self) { 
      int i = ReadToken(self->I, 1);

  return (! sflag) && (i < 0);  }
bool limit_min_guard (actor_Clip_source *self) { 
      int i = ReadToken(self->I, 1);

  return (i < ( - 255) );  }

 inline void  read_signed  (actor_Clip_source *self) { 
    int s = ConsumeToken(self->SIGNED, 1);
  sflag = s;
  count = 63;
  }
 inline void  limit_max  (actor_Clip_source *self) { 
    int i = ConsumeToken(self->I, 1);
  count = (count - 1);
  SendToken(self->O, 255, 1);
  }
 inline void  limit_zero  (actor_Clip_source *self) { 
    int i = ConsumeToken(self->I, 1);
  count = (count - 1);
  SendToken(self->O, 0, 1);
  }
 inline void  limit_min  (actor_Clip_source *self) { 
    int i = ConsumeToken(self->I, 1);
  count = (count - 1);
  SendToken(self->O, ( - 255) , 1);
  }
 inline void  limit_none  (actor_Clip_source *self) { 
    int i = ConsumeToken(self->I, 1);
  count = (count - 1);
...
static void run(actor_Clip_source *self)
{
...
static bool not_finished(actor_Clip_source* self)
{
    return might_has_input(self->I)
        || network_not_finished(&Mailbox.n_source[1]);
}
static void end(actor_Clip_source *self)
{
     end_port(self->O);
}
void actor_Clip_source_init(actor_Clip_source *self)
{
