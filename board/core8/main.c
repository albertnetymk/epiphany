...
{
    all.idctCol_instance_Combine = a;
    actor_Combine_init(a);
    all.idctCol_instance_Combine->row = false;
    api.run = (run_t *)all.idctCol_instance_Combine->run;
    api.end = (end_t *)all.idctCol_instance_Combine->end;
    api.not_finished = (not_finished_t *)all.idctCol_instance_Combine->not_finished;
    return &api;
}
int main(void) {
...
#endif
    instance_Combine.X0 = address_from_coreid(mycoreid, &X0);
    instance_Combine.X1 = address_from_coreid(mycoreid, &X1);
    instance_Combine.X2 = address_from_coreid(mycoreid, &X2);
   ...
   core_main(address_from_coreid(mycoreid, &instance_Combine), &init);
   return 0;
}
