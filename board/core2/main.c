...
{
    all.idctRow_instance_Combine = a;
    actor_Combine_init(a);
    all.idctRow_instance_Combine->row = true;
    api.run = (run_t *)all.idctRow_instance_Combine->run;
    api.end = (end_t *)all.idctRow_instance_Combine->end;
    api.not_finished = (not_finished_t *)all.idctRow_instance_Combine->not_finished;
    return &api;
}
int main(void) {
