...
{
    all.instance_ShuffleFly = a;
    actor_ShuffleFly_init(a);
    api.run = (run_t *)all.instance_ShuffleFly->run;
    api.end = (end_t *)all.instance_ShuffleFly->end;
    api.not_finished = (not_finished_t *)all.instance_ShuffleFly->not_finished;
    return &api;
}
int main(void) {
