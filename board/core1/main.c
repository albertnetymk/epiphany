...
{
    all.idctRow_instance_Scale = a;
    actor_Scale_init(a);
    api.run = (run_t *)all.idctRow_instance_Scale->run;
    api.end = (end_t *)all.idctRow_instance_Scale->end;
    api.not_finished = (not_finished_t *)all.idctRow_instance_Scale->not_finished;
    return &api;
}
int main(void) {
