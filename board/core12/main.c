...
{
    all.instance_Shift = a;
    actor_Shift_init(a);
    api.run = (run_t *)all.instance_Shift->run;
    api.end = (end_t *)all.instance_Shift->end;
    api.not_finished = (not_finished_t *)all.instance_Shift->not_finished;
    return &api;
}
int main(void) {
...
#endif
    instance_Shift.X0 = address_from_coreid(mycoreid, &X0);
    instance_Shift.X1 = address_from_coreid(mycoreid, &X1);
    instance_Shift.X2 = address_from_coreid(mycoreid, &X2);
    instance_Shift.X3 = address_from_coreid(mycoreid, &X3);
    instance_Shift.Y0 = address_from_coreid(mycoreid, &Y0);
    instance_Shift.Y1 = address_from_coreid(mycoreid, &Y1);
    instance_Shift.Y2 = address_from_coreid(mycoreid, &Y2);
    instance_Shift.Y3 = address_from_coreid(mycoreid, &Y3);
   core_main(address_from_coreid(mycoreid, &instance_Shift), &init);
   return 0;
}
